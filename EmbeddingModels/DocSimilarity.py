...
docEmb = embeddings.embed_documents(docs)
queryEmb = embeddings.embed_query(query)
similarity = cosine_similarity([queryEmb], docEmb)[0]

print(sorted(list(enumerate(similarity)), key=lambda x:x[1])[-1])

# Or
# for i in range(len(docs)):
#     print(f"Document: {i} -> Similarity: {similarity[i]}")